...
import io.nshusa.app.osdc.model.ItemDefJson
...
open class Controller : BaseController() {
            ...
            override fun call(): Boolean {
                try {
                    Cache(FileStore.open(cachePath)).use { cache ->
                        val table = cache.getReferenceTable(CacheIndex.SKELETONS)

                        val skeletons = arrayOfNulls<Array<Skeleton?>>(table.capacity())

                        for (mainSkeletonId in 0 until table.capacity()) {

                            val bos = ByteArrayOutputStream()

                            DataOutputStream(bos).use { dos ->
								try {
	                                val archive = Archive.decode(cache.read(CacheIndex.SKELETONS, mainSkeletonId).data, table.getEntry(mainSkeletonId).size())
                            ...
                            val progress = (mainSkeletonId + 1).toDouble() / table.capacity() * 100

                            updateMessage(String.format("%.2f%s", progress, "%"))
                            updateProgress((mainSkeletonId + 1).toDouble(), table.capacity().toDouble())
		...
		val task = object: Task<Boolean>() {
            override fun call(): Boolean {
                Cache(FileStore.open(cachePath)).use { cache ->
                    ...
                    val size = list.size()
                    val defJson = ArrayList<ItemDefJson>()
                    for (i in 0 until size) {
                        val def = list.list(i) ?: continue
                        ...
                        try {
							if(def.note != -1 && def.price == 1){
								val counterpart = list.list(def.note)
								defJson.add(ItemDefJson(def.getId(), counterpart.name + "[N]", def.note, counterpart.price, true, "SimpleItemDefinition"))
							} else {
								defJson.add(ItemDefJson(def.getId(), def.name, def.note, def.price, def.stackable == 1, "SimpleItemDefinition"))
							}
                            count++
                        } catch (ex: Exception) {
                    ...
                    FileWriter(File(dir, "itemdefs.json")).use { writer ->
                        var gson = GsonBuilder().setPrettyPrinting().create()
						
						gson.toJson(defJson, writer)
                    }
    ...
    private fun scrapXTEAS() {
        ...
        val task = object:Task<Boolean>() {
            override fun call(): Boolean {
                val tagUrl = "https://api.github.com/repos/runelite/runelite/tags"

                try {
                    var connection =  URL(tagUrl).openConnection() as HttpURLConnection
                    connection.requestMethod = "GET"

                    if (connection.responseCode != 200) {
                        Platform.runLater {
                            Dialogue.showWarning("Failed to retrieve latest tag from $tagUrl").show()
                        }
                        return false
                    }
                    ...
                    BufferedReader(InputStreamReader(connection.inputStream)).useLines { line ->
                        for (itr in line.iterator()) {
                            builder.append(itr)
                        }
                    }

                    val gson = GsonBuilder().create()
                    val tags = gson.fromJson(builder.toString(), Array<GithubTag>::class.java)

                    if (tags.isEmpty()) {
                        return false
                    }

                    val tagName = tags[0].name

                    val versionTag = tagName.substring(tagName.lastIndexOf("-") + 1, tagName.length)

                    val xteaUrl = "https://api.runelite.net/runelite-$versionTag/xtea"

                    connection = URL(xteaUrl).openConnection() as HttpURLConnection
                    connection.requestMethod = "GET"
                    ...
                    BufferedReader(InputStreamReader(connection.inputStream)).use { reader ->
                        ...
                        for (i in 0 until xteas.size) {
                            val xtea = xteas[i]

                            PrintWriter(FileWriter(File(outputDir, "${xtea.region}.txt"))).use { writer ->
                                for (key in xtea.keys) {
                                    writer.println(key)
                                }
                                count++
                            }

                            val progress = (i + 1).toDouble() / xteas.size * 100

                            updateMessage(String.format("%.2f%s", progress, "%"))
                            updateProgress((i + 1).toDouble(), xteas.size.toDouble())
                        }
    ...
    private fun dumpMaps() {
        ...
        chooser.title = "Select directory containing xteas"
        chooser.initialDirectory = File("./")

        val xteaDir = chooser.showOpenDialog(App.stage) ?: return

        if (!XTEAManager.load(xteaDir)) {
            Dialogue.showWarning("Could not load any xteas in directory ${xteaDir.path}.").show()
            return
        }
    ...
    private fun dumpMapImage() {
        ...
        val xteaChooser = FileChooser()
        xteaChooser.title = "Select directory containing xteas"
        xteaChooser.initialDirectory = File("./")
        val xteaDir = xteaChooser.showOpenDialog(App.stage) ?: return
        ...
        runTask("Map Image Task", MapImageTask(cachePath!!.toPath(), xteaDir.toPath(), outputDir.toPath()))
    }
